---
name: fullstack-flow/phases/phase-6.5-rule-sync
...
1. Read `.codebuddy/workflow/state.yaml`，验证 `phase.current == "phase-6.5-rule-sync"`
...
2. Phase 出口：
   ...
   - `progress.phases_completed.append("phase-6.5-rule-sync")`
   - `phase.current = "phase-6.6-audit"`, `phase.status = "pending"`
   - `metrics_snapshot.phase_durations[phase-6.5-rule-sync] = 耗时分钟数`
